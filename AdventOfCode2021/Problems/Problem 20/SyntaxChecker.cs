...
namespace AdventOfCode2021.Problems.Problem20
{
    public class SyntaxChecker
    {
        ...
        public List<List<char>> GetMissingClosingCharactersForLines()
        {
            if(_subsystemLines.Count > 0)
            {
                List<List<char>> missingClosingCharactersForLines = new();
                foreach(var line in _subsystemLines)
                {
                    if(line.IsValid && !line.IsClosed)
                    {
                        missingClosingCharactersForLines.Add(line.FindClosingCharacters());
                    }
                }
                return missingClosingCharactersForLines;
            }
            else
            {
                return new();
