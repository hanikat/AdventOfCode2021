...
namespace AdventOfCode2021.Problems.Problem22
{
    public class DumboOctupusSchool
    {
        ...
        public int CountNumberOfStepsBeforeSimultaneousFlash()
        {
            bool allOctupusesFlashed = false;
            int steps = 0, accumulatedFlashes = 0;

            while(!allOctupusesFlashed)
            {
                int totalFlashes = CountFlashesAfterSteps(1);
                steps++;
                int numberOfFlashesInStep = totalFlashes - accumulatedFlashes;
                accumulatedFlashes = totalFlashes;

                if(numberOfFlashesInStep == _octupusSchool.Length)
                {
                    allOctupusesFlashed = true;
                }
            }
